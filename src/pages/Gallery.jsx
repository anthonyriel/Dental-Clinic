  ...
  return (
    <div className="page-stack">
      <div className="public-page-heading">
        <span className="eyebrow">A SPACE TO FEEL AT EASE</span>
        <h1>Get to know<br/>your <em>smile space.</em></h1>
        <p>A look inside Dentaprime. We look forward to welcoming you in person.</p>
      </div>

      <div className="filter-chips justify-center" style={{ gap: '10px', flexWrap: 'wrap' }}>
      ...
      {gallery.loading ? (
        <p className="empty-state">Loading the clinic gallery...</p>
      ) : !gallery.error && items.length ? (
        <div className="service-grid">
          {items.map(item => (
            <figure key={item.id} className="glass-panel gallery-card">
              <div>
                <img 
                  loading="lazy" 
                  src={item.image_url} 
              ...
              <figcaption>
                <span className="eyebrow">{item.category?.replaceAll('_', ' ') || 'OUR CLINIC'}</span>
                <h2>{item.title || 'Inside Dentaprime'}</h2>
              </figcaption>
            </figure>
          ))}
        </div>
        ...
        <div className="glass-panel empty-state">
          <Image className="mx-auto mb-4" size={30} strokeWidth={1.3}/>
          <h2>A closer look is coming.</h2>
          <p>No clinic photos are listed in this category yet.</p>
